...
/*
	...
	License: GPLv2 - http://github/licenses/gpl-2.0.txt
*/
...
class CoinPaymentsAPI {
	 ...
	 * @param ipn_url Optionally set an IPN handler to receive notices about this transaction. If ipn_url is empty then it will use the default IPN URL in your account.
	 */
	public function GetCallbackAddress($currency, $label, $ipn_url = '') {		
		$req = array(
			'currency' => $currency,
			'label' => $label,
			'ipn_url' => $ipn_url,
	 ...
	 * @param ipn_url Optionally set an IPN handler to receive notices about this transaction. If ipn_url is empty then it will use the default IPN URL in your account.
	 */
	public function CreateWithdrawal($amount, $currency, $address, $auto_confirm = FALSE, $ipn_url = '', $note) {		
		$req = array(
			'amount' => $amount,
			'currency' => $currency,
			'address' => $address,
